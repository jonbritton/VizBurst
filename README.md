...
3. Uploads use high-concurrency multipart transfer (s5cmd/rclone), with many-small-file asset sets packed into archives — single-stream copies won't saturate the link, especially on texture-heavy shots.
4. Cloud workers pull assets from S3 to **instance storage** at task start via a **VPC gateway endpoint** (no public internet! no money down!) The local cache is keyed to the manifest hash to minimize downloads.
